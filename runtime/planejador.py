...
def _obter_config_llm() -> dict:
    """Resolve configuracao de LLM a partir do ambiente.

    Suporta OpenAI puro e provedores compativeis com a API da OpenAI,
    incluindo OpenRouter.
    """
    chave_api = (
        os.environ.get("OPENAI_API_KEY")
        or os.environ.get("OPENROUTER_API_KEY")
        or os.environ.get("Open-Router")
    )

    if not chave_api:
        return {}

    usando_openrouter = bool(
        os.environ.get("OPENROUTER_API_KEY")
        or os.environ.get("Open-Router")
    )

    base_url = os.environ.get("OPENAI_BASE_URL") or os.environ.get("OPENROUTER_BASE_URL")
    modelo = os.environ.get("OPENAI_MODEL") or os.environ.get("OPENROUTER_MODEL")
    headers = {}

    if usando_openrouter:
        base_url = base_url or "https://openrouter.ai/api/v1"
        modelo = modelo or "openai/gpt-4o-mini"
...
def chamar_llm(percepcao: str, contratos: dict, historico: list = None) -> tuple:
    ...
    cliente = OpenAI(
        ...
        default_headers=config_llm.get("default_headers"),
    )
